...
def generate_progress_bar(data, output_path):
    ...
    ax.set_ylim(0, 10)  # Adjust the y-axis limit to increase the "height" of the progress bar area
    ...
    for i, reading in enumerate(core_readings):
        ...
        rect = patches.FancyBboxPatch((current_time, 2.5), reading_times[i], height, 
                                      boxstyle=f"round,pad={pad}", color=color, ec="none")
        ax.add_patch(rect)
        # Include reading time in the text annotation
        text_label = f"{reading['title']}\n({reading_times[i]} min)"
        plt.text(current_time + reading_times[i] / 2, 8, text_label, ha='center', va='bottom', rotation=45, fontsize=8, wrap=True)
