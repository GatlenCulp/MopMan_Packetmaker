...
def mergePdfs(pdf_paths:list, output_path:pl.Path, merge_on_odd:bin=True) -> pl.Path:
...
def getPrecontext(curriculum_id, output_dir, option_num:int=1) -> dict:
    if option_num == 1:
        precontext = getPrecontextForCurriculum(curriculum_id, output_dir/pl.Path("precontext"))
        return precontext
    elif option_num == 2:
...
def main(curriculum_id:str, output_dir:pl.Path=pl.Path("./output/")) -> None:
    ...
    if config["generate"]["cover"]:
        cover = CoverGenerator(pl.Path(config["templates"]["cover"]), output_dir/pl.Path("Cover"), precontext, overwrite=True)
    ...
    if config["generate"]["device_readings"]:
        for reading in precontext["core_readings"]:
            if not reading["trimmed_pdf"] and not reading["read_on_device"]: raise ValueError(f"Reading {reading['title']} has no trimmed pdf and is not labeled as read_on_device.")
            if not reading["read_on_device"]: continue
            ...
            device_reading = DeviceReadingGenerator(
                ...
                output_dir/pl.Path(f"Device Readings/{makeIDFromTitle(reading['title'])}"),
                precontext,
                overwrite=True
            )
    ...
    if precontext["further_readings"] and config["generate"]["further_readings"]:
        further = FurtherGenerator(pl.Path(config["templates"]["further_reading"]), output_dir/pl.Path("Further"), precontext, overwrite=True)
        further_pdf_path = further.pdf_path

    ## Merge Cover + Readings + Further
    if config["generate"]["packet"]:
        print("\n")
        logger.info("Merging cover, core readings, and further reading page into packet...")
        reading_pdf_paths = [pl.Path(reading["trimmed_pdf"]) for reading in precontext["core_readings"]]
        packet_path = mergePdfs(
            ([cover_pdf_path] if cover_pdf_path else []) + reading_pdf_paths + ([further_pdf_path] if further_pdf_path else []),
            output_path=output_dir/pl.Path(makeIDFromTitle(precontext["curriculum_name"]) + ".pdf")
        )
        packet_path = add_footer_to_pdf(packet_path, packet_path, footer_text=precontext["program_name"] + " Readings — Page {i} of {n}")
    ...
    if config["generate"]["tas_guides"]:
        ...
        for cohort in precontext["cohorts"]:
            ...
            guide = GuideGenerator(guide_template_path, guide_dir, precontext, overwrite=True)
            meeting_ta_guide_pdf = [pl.Path(precontext["meeting_ta_guide_pdf"])] if precontext["meeting_ta_guide_pdf"] else []
            guide_pdfs = [guide.pdf_path] + meeting_ta_guide_pdf + [pl.Path(precontext["base_ta_guide_pdf"])]
            guide_path = mergePdfs(guide_pdfs, output_path=ta_guide_output_dir/pl.Path(guide_name + ".pdf"))
            logger.info(f"[SUCCESS] {guide_path}")
        logger.info("[SUCCESS] All TA guides generated.")


if __name__ == "__main__":
    for curriculum in config["curriculum_to_generate"]:
        curriculum_id = config["curriculum_ids"][curriculum]
        output_dir = pl.Path(config["output_dir"])/pl.Path(makeIDFromTitle(curriculum))
