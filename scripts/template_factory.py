...
def adjustLogo(logo_path: pl.Path, output_path: pl.Path=pl.Path("./")) -> pl.Path:
    ...
    if 'cropped' in logo_path.stem:
        ...
        im = trim(im)
        cropped_logo_path = output_path / pl.Path(f"{logo_path.stem}_cropped{logo_path.suffix}")
...
def makeQRCode(url: str, title: str, output_path: pl.Path=pl.Path("./")) -> pl.Path:
    ...
    qr = qrcode.QRCode(version=5, box_size=20, border=1, error_correction=qrcode.constants.ERROR_CORRECT_H)
    qr.add_data(url)
    qr.make(fit=True)
...
def makeIDFromTitle(title):
    ...
    return ''.join(char.lower() for char in title if char.isalnum() or char == ' ').replace(" ", "_")
