...
env = dotenv.dotenv_values(".env/.env")
...
def getPrecontextForCurriculum(curriculum_id: str, output_dir:pl.Path=pl.Path("./precontexts")) -> dict:
    ...
    def getFromRecord(record:dict, field:str, attachment_file_path:pl.Path=None, single_attachment:bool=True):
        ...
        if attachment_file_path:
            attachment_paths = [
                str(save_from_airtable(attachment["url"], attachment_file_path.with_stem(f"{attachment_file_path.stem}{i}")))
    ...
    core_readings = [{
        ...
        "trimmed_pdf": getFromRecord(
            reading, "trimmed_pdf", 
            attachment_file_path=output_dir/pl.Path(makeIDFromTitle(getFromRecord(reading, "title") + getFromRecord(reading, "subsection")), single_attachment=True)
        ),
    ...
    further_readings = [mopman.get(at_map["readings"], id) for id in further_reading_ids]
    ...
    cohorts = list(map(lambda cohort: {
    ...
    precontext = {
        ...
        "logo_path": getFromRecord(org, "logo_master_raster", attachment_file_path=output_dir/pl.Path("logo"), single_attachment=True),
        ...
        "further_readings": further_readings,
        "cohorts": cohorts,
        "meeting_ta_guide_pdf": getFromRecord(curriculum, "meeting_ta_guide_pdf", attachment_file_path=output_dir/pl.Path("TA Guides/meeting_ta_guide"), single_attachment=True),
        "base_ta_guide_pdf": getFromRecord(org, "base_ta_guide_pdf", attachment_file_path=output_dir/pl.Path("TA Guides/base_ta_guide"), single_attachment=True),
...
if __name__ == "__main__":
    ...
    precontext = getPrecontextForCurriculum(target_curriculum_id, pl.Path("./precontexts/test1"))
    print(precontext)
