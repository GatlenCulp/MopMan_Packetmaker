"""
gui.py
Generates a GUI for editing the configuration file and making packets from the configuration.
"""
...
class ConfigEditor:
    ...
    def make_packets(self):
        ...
        messagebox.showinfo(
            "Please Wait",
            "Your packets are being made. This may take a while. Please wait...",
        )
    ...
    def init_ui(self):
        ...
        self.notebook.pack(
            fill=tk.BOTH, expand=True, side=tk.TOP, padx=10, pady=(10, 0)
        )
        ...
        # Make Packets button
        make_packets_button = ttk.Button(
            self.buttons_frame, text="Make Packets", command=self.make_packets
        )
    ...
    def init_curriculum_ui(self):
        self.tree = ttk.Treeview(
            ...
            columns=("Curriculum Name", "Record ID", "Make Packet"),
            show="headings",
        )
        ...
        add_curriculum_button = ttk.Button(
            self.curriculum_frame,
            text="Add New Curriculum",
            command=self.add_new_curriculum,
        )
        add_curriculum_button.pack(side=tk.BOTTOM, pady=10)

        # Delete Curriculum button
        delete_curriculum_button = ttk.Button(
            self.curriculum_frame,
            text="Delete Curriculum",
            command=self.delete_curriculum,
        )
    ...
    def add_new_curriculum(self):
        ...
        curriculum_name = simpledialog.askstring(
            "New Curriculum", "Enter the curriculum name:", parent=self.master
        )
        if not curriculum_name:  # User cancelled or entered an empty name
            return
        record_id = simpledialog.askstring(
            "New Curriculum", "Enter the record ID:", parent=self.master
        )
        if not record_id:  # User cancelled or entered an empty ID
            return
        make_packet = messagebox.askyesno(
            "New Curriculum",
            "Make packet for this curriculum?",
            icon="question",
            parent=self.master,
        )

        # Update the config dictionary
        self.config["curriculum"][curriculum_name] = {
            "record_id": record_id,
    ...
    def delete_curriculum(self):
        ...
        if selected_items:  # Check if there is a selected item
            ...
            if messagebox.askyesno(
                "Delete Curriculum", "Are you sure you want to delete this curriculum?"
            ):
    ...
    def populate_treeview(self):
        ...
        for meeting, details in self.config["curriculum"].items():
            ...
            self.tree.insert(
                ...
                tk.END,
                iid=meeting,
                values=(meeting, details["record_id"], make_packet),
            )
    ...
    def init_templates_ui(self):
        for template_name, template_path in self.config["templates"].items():
            ...
            entry.bind(
                "<FocusOut>",
                lambda e, name=template_name: self.update_template_path(
                    name, e.widget.get()
                ),
            )
            entry.bind(
    ...
    def init_generate_ui(self):
        for option, value in self.config["generate"].items():
            ...
            check_button.bind(
                "<Button-1>",
                lambda e, name=option, var=check_var: self.update_generate_option(
                    name, var
                ),
            )
